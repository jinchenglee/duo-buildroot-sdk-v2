...
- **Not run on real hardware.** The model passes host simulation and the binary
  cross-compiles and links against the right libraries, but neither has been
  executed on a board. `run_tinytag.sh --selftest` is the first thing to run
  after flashing — it is exactly the check that would catch a
  simulator-versus-silicon discrepancy.
